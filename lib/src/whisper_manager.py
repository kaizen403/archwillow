...
class WhisperManager:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if backend == 'realtime-ws':
                ...
                # Initialize the appropriate realtime client
                ...
                if provider_id == 'elevenlabs':
                    ...
                    # Apply ElevenLabs-specific config
                    el_cfg = self.config.get_setting('elevenlabs_realtime', {})
                    self._realtime_client.audio_format = el_cfg.get('audio_format', 'pcm_16000')
                    self._realtime_client.commit_strategy = el_cfg.get('commit_strategy', 'vad')
                    self._realtime_client.no_verbatim = el_cfg.get('no_verbatim', True)
                    self._realtime_client.include_timestamps = el_cfg.get('include_timestamps', False)
                    self._realtime_client.include_language_detection = el_cfg.get('include_language_detection', False)
                    self._realtime_client.region = el_cfg.get('region', 'default')
                    self._realtime_client.vad_silence_threshold_secs = el_cfg.get('vad_silence_threshold_secs', 1.5)
                    self._realtime_client.vad_threshold = el_cfg.get('vad_threshold', 0.4)
                    self._realtime_client.min_speech_duration_ms = el_cfg.get('min_speech_duration_ms', 100)
                    self._realtime_client.min_silence_duration_ms = el_cfg.get('min_silence_duration_ms', 100)
                    self._realtime_client.enable_logging = el_cfg.get('enable_logging', True)
                    self._realtime_client.keyterms = el_cfg.get('keyterms', [])
                else:
                    # OpenAI Realtime API
                ...
                # Build instructions from whisper_prompt and language (OpenAI only)
                instructions_parts = []
                ...
                # Set language in realtime client
                self._realtime_client.language = language
                
                # ElevenLabs sample rate matches hyprwhspr capture rate
                if provider_id == 'elevenlabs':
                    self._realtime_client.sample_rate = 16000
                
                # Set buffer max seconds (OpenAI only)
                if provider_id != 'elevenlabs':
                ...
                if provider_id == 'elevenlabs':
                    # hyprwhspr captures at 16kHz, ElevenLabs default is pcm_16000
                    self._realtime_streaming_callback = self._realtime_client.append_audio
                else:
                    # OpenAI requires 24kHz
    ...
    def _get_websocket_url(self, provider_id: str, model_id: str, mode: str = 'transcribe') -> str:
        ...
        # Build query parameters based on provider and mode
        if provider_id == 'elevenlabs':
            return self._build_elevenlabs_url(base_url, model_id)
    ...
    def _build_elevenlabs_url(self, base_url: str, model_id: str) -> str:
        """Build ElevenLabs Scribe v2 Realtime WebSocket URL with query parameters."""
        from urllib.parse import urlencode

        el_cfg = self.config.get_setting('elevenlabs_realtime', {})

        params = [('model_id', model_id)]
        params.append(('audio_format', el_cfg.get('audio_format', 'pcm_16000')))
        params.append(('commit_strategy', el_cfg.get('commit_strategy', 'vad')))

        if el_cfg.get('include_timestamps', False):
            params.append(('include_timestamps', 'true'))
        if el_cfg.get('include_language_detection', False):
            params.append(('include_language_detection', 'true'))

        language = el_cfg.get('language_code') or self.config.get_setting('language', None)
        if language:
            params.append(('language_code', language))

        if el_cfg.get('no_verbatim', True):
            params.append(('no_verbatim', 'true'))

        for term in el_cfg.get('keyterms', []):
            params.append(('keyterms', term))

        if el_cfg.get('vad_silence_threshold_secs') is not None:
            params.append(('vad_silence_threshold_secs', el_cfg['vad_silence_threshold_secs']))
        if el_cfg.get('vad_threshold') is not None:
            params.append(('vad_threshold', el_cfg['vad_threshold']))
        if el_cfg.get('min_speech_duration_ms') is not None:
            params.append(('min_speech_duration_ms', el_cfg['min_speech_duration_ms']))
        if el_cfg.get('min_silence_duration_ms') is not None:
            params.append(('min_silence_duration_ms', el_cfg['min_silence_duration_ms']))

        if not el_cfg.get('enable_logging', True):
    ...
    def _transcribe_realtime(self, _audio_data: np.ndarray, _sample_rate: int = 16000, language_override: Optional[str] = None) -> str:
        ...
        try:
            ...
            print(f'[REALTIME] Transcription result: {transcription!r}', flush=True)
    ...
    def get_current_model(self) -> str:
        ...
        if self.current_model is None:
            ...
            if backend == 'rest-api':
                return ''
            if backend == 'realtime-ws':
                return self.config.get_setting('websocket_model', '')
    ...
    def get_backend_info(self) -> str:
        ...
        if backend == 'realtime-ws':
            provider_id = self.config.get_setting('websocket_provider', 'unknown')
            model_id = self.config.get_setting('websocket_model', 'unknown')
            return f"Realtime WebSocket ({provider_id} / {model_id})"
        
        if backend == 'onnx-asr':
            model_name = self.config.get_setting('onnx_asr_model', 'unknown')
            return f"ONNX-ASR ({model_name})"
